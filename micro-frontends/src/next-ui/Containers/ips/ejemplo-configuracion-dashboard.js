/**
 * Ejemplo de configuración para integrar el IPS Display Control en Bahmni
 * 
 * Este archivo muestra cómo configurar el dashboard IPS en diferentes contextos:
            ...
            var setupIpsData = function() {
                $scope.ipsData = {
                    ...
                    provider: {
                        ...
                        display: $scope.currentProvider.person ? $scope.currentProvider.person.display : $scope.currentProvider.display
                    },
                        ...
                        generateDocument: function(patientUuid) {
                            return ipsService.generateDocument(patientUuid).then(function(response) {
                                if (response.data && response.data.documentUrl) {
                                    ...
                                    link.download = 'IPS_' + $scope.patient.identifier + '_' + new Date().toISOString().split('T')[0] + '.pdf';
                                    link.click();
                                }
                                return response;
...
// ========================================
// 7. EJEMPLO DE USO EN DIFERENTES CONTEXTOS
// ========================================
