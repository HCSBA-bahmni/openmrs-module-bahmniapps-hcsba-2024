...
// Config centralizada (valores por defecto + override)
const { REGIONAL_BASE, VHL_ISSUANCE_URL, VHL_RESOLVE_URL, ICVP_BASE } = ICVP_CONFIG;
const BASIC_AUTH = buildBasicAuth();
    "Basic " + (typeof btoa === "function" ? btoa(`${BASIC_USER}:${BASIC_PASS}`) : "");

/* VHL issuance/resolve URLs vienen de la config */

/* ICVP_BASE provisto en la config */
...
// Extrae Bundle/{id} desde attachment.url del DocumentReference
const extractBundleIdFromDocRef = (doc) => {
    const attUrl = doc?.content?.[0]?.attachment?.url || "";
    const m = String(attUrl).match(/\/?Bundle\/([A-Za-z0-9\-.]+)/);
    return m ? m[1] : null;
};
...
export function IpsIcvpDisplayControl(props) {
    ...
    const [vhlInput, setVhlInput] = useState("");
    ...
    // Cleanup general (detener escáner si el componente se desmonta)
    useEffect(() => {
        return () => {
            try {
    ...
    /* -------- NUEVO: Modal y estado para ICVP -------- */
    const [icvpOpen, setIcvpOpen] = useState(false);
    const [icvpLoading, setIcvpLoading] = useState(false);
    const [icvpError, setIcvpError] = useState(null);
    const [icvpPng, setIcvpPng] = useState("");  // data:image/png;base64,...
    const [icvpHc1, setIcvpHc1] = useState("");  // "HC1:..."
    ...
    /* -------- NUEVO: Generar ICVP a partir del DocumentReference seleccionado -------- */
    const handleGenerateICVP = async (doc) => {
        setIcvpOpen(true);
        setIcvpLoading(true);
        setIcvpError(null);
        setIcvpPng("");
        ...
        try {
            const bundleId = extractBundleIdFromDocRef(doc);
            if (!bundleId) throw new Error("No se pudo derivar el Bundle ID desde el DocumentReference.");

            const url = `${ICVP_BASE}/fhir/Bundle/${encodeURIComponent(bundleId)}/$icvp`;
            const res = await axios.get(url, {
                headers: {
                    // Si tu backend $icvp también está detrás de OpenHIM con Basic, puedes usar:
            ...
            const dr = batch?.entry?.[0]?.resource;
            if (!dr || dr.resourceType !== "DocumentReference") {
                throw new Error("Respuesta inesperada: no llegó un DocumentReference dentro del Bundle.");
            }

            const pngPart = (dr.content || []).find(c => c?.attachment?.contentType === "image/png");
            // Corregido: el MIME correcto es "text/plain" (antes se buscaba "plain/text")
            const hc1Part = (dr.content || []).find(c => String(c?.attachment?.contentType || "").startsWith("text/plain"));

            const pngB64 = pngPart?.attachment?.data || "";
            const hc1 = hc1Part?.attachment?.data || "";

            if (!pngB64 || !hc1) {
                throw new Error("El DocumentReference no contiene PNG y/o texto HC1.");
            }

            setIcvpPng(`data:image/png;base64,${pngB64}`);
            setIcvpHc1(hc1);
        } catch (e) {
            const msg = e?.response ? `${e.response.status} ${e.response.statusText}` : (e?.message || String(e));
    ...
    return (
        <I18nProvider>
            <div className="ips-display-control ips-icvp-display-control">
                {/* Header */}
                <div className="ips-header">
                    ...
                    <Button kind="primary" renderIcon={QrCode32} onClick={handleOpenVhlReader}>
                        <FormattedMessage id="READ_ICVP_DOCUMENT" defaultMessage="Leer ICVP/DVC"/>
                    </Button>
                </div>
                                                                            ...
                                                                            return (
                                                                                <TableCell key={cell.id}>
                                                                                    <div style={{display:"flex", gap:"0.25rem"}}>
                                                                                        ...
                                                                                        {/* NUEVO: Generar ICVP desde el DocumentReference */}
                                                                                        <Button
                                                                                            kind="ghost"
                                                                                            size="sm"
                                                                                            renderIcon={QrCode32}
                                                                                            disabled={!docForRow}
                                                                                            onClick={() =>
                                                                                                docForRow && handleGenerateICVP(docForRow)
                                                                                            }
                                                                                        >
                                                                                            <FormattedMessage id="GENERATE_ICVP"
                                                                                                              defaultMessage="Generar ICVP"/>
                                                                                        </Button>
                                                                                    </div>
                                                                                </TableCell>
                                                                            );
                ...
                <ComposedModal open={vhlModalOpen} onClose={handleCloseVhlModal} size="lg">
                    <ModalHeader label="ICVP/DVC" title={tx?.("READ_ICVP_DOCUMENT") || "Leer ICVP/DVC"}/>
                    <ModalBody hasScrollingContent>
                        <div className="vhl-reader" style={{display: "grid", gap: "1rem"}}>
                            <TextArea
                                id="vhl-input"
                                labelText={tx?.("PASTE_ICVP_HC1") || "Pega el código ICVP/DVC (HC1)"}
                                placeholder="HC1:..."
                                rows={4}
                                value={vhlInput}
                            ...
                            <div>
                                <Button kind="primary" size="sm" onClick={handleResolveVHL} disabled={resolveLoading}>
                                    {resolveLoading ? (
                                        <InlineLoading
                                            description={tx?.("RESOLVING_ICVP") || "Resolviendo ICVP/DVC..."}
                                        />
                                    ) : (
                                        <FormattedMessage id="RESOLVE_ICVP" defaultMessage="Resolver ICVP/DVC"/>
                                    )}
                                </Button>
                                {resolveError && (
                ...
                {/* NUEVO: Modal ICVP (QR + HC1) */}
                <ComposedModal open={icvpOpen} onClose={() => setIcvpOpen(false)} size="sm">
                    <ModalHeader label="ICVP" title="Certificado de Inmunización" />
                    <ModalBody hasScrollingContent>
                        {icvpLoading && (
                        ...
                        {!icvpLoading && icvpError && (
                            <div style={{color:"#da1e28"}}>{icvpError}</div>
                        )}

                        {!icvpLoading && !icvpError && (
                            <div style={{display:"grid", gap:"0.75rem", justifyItems:"center"}}>
                                {icvpPng && <img src={icvpPng} alt="QR ICVP" style={{width:192, height:192}} />}
                                {icvpHc1 && (
                                    <>
                                        <div style={{
                                            whiteSpace:"pre-wrap", wordBreak:"break-word",
                                            background:"var(--cds-layer, #f4f4f4)", padding:"0.75rem",
                                            borderRadius:"0.25rem", fontFamily:"monospace", fontSize:"0.825rem"
                                        }}>
                                            {icvpHc1}
                                        </div>
                                        <div style={{display:"flex", gap:"0.5rem"}}>
                                            <Button kind="secondary" size="sm" onClick={()=>{
                                                ...
                                                a.href=icvpPng; a.download="icvp-qr.png"; a.click();
                                            }}>
                                                <FormattedMessage id="DOWNLOAD_PNG" defaultMessage="Descargar PNG" />
                                            </Button>
                                            <Button kind="secondary" size="sm" onClick={async()=>{
                                                try{ await navigator.clipboard.writeText(icvpHc1);}catch{}
                                            }}>
                    ...
                    <ModalFooter>
                        <Button kind="secondary" onClick={() => setIcvpOpen(false)}>
                            <FormattedMessage id="CLOSE" defaultMessage="Cerrar" />
                        </Button>
                    </ModalFooter>
                </ComposedModal>
